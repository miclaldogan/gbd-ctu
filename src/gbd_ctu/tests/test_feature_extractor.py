...
def test_flow_feature_columns_has_22_entries() -> None:
    """FLOW_FEATURE_COLUMNS must contain exactly 22 feature names (regression guard)."""
    assert len(FLOW_FEATURE_COLUMNS) == 22
...
def test_feature_names_property_returns_22_strings() -> None:
    """extractor.feature_names must be the list of 22 feature name strings."""
    extractor = FlowFeatureExtractor()
...
def test_transform_splits_returns_correct_keys_and_shapes() -> None:
    ...
    assert result["train"].shape == (2, 22)
    assert result["val"].shape == (1, 22)
    assert result["test"].shape == (1, 22)
